'''
(2017 NIPS) Simple and Scalable Predictive Uncertainty Estimation using Deep Ensembles. Lakshminarayanan et al.
(2019 PhD Thesis) Uncertainty Estimation in Deep Learning with application to Spoken Language Assessment. Malinin.
'''
import torch
...
import os
# revise the level and data path (level influences model_path)
level = "easy" # choose from entry, easy, medium, hard
...
for i in range(T):
    model_path = f"./model_used/mobilenet_{level}_{i}.pth"
...
pd = np.argmax(probs_mean, axis=1)
gt = np.empty((0,), dtype=np.int32)
with torch.no_grad():
    for X,y in test_dataloader:
        gt = np.append(gt, y.numpy())

tu = h(probs_mean)  # total uncertainty
print(tu.shape)
datasize = tu.shape[0]
